...
import { getSeoFamiliaLanding } from "../../../lib/seoLandingApi";
...
async function loadLanding(slug, page = 1) {
  try {
    const response = await getSeoFamiliaLanding(slug, { page, limit: 12 }, { next: { revalidate: 1800 } });
...
export async function generateMetadata({ params }) {
  ...
  const description = cleanText(
    ...
    `Consulta productos de la línea ${landing.nombre} en Andyfers.`
  ).slice(0, 280);
  const canonical = `/catalogo/familia/${encodeURIComponent(landing.slug || slug)}`;
...
function LandingJsonLd({ data, slug }) {
  ...
  const canonical = siteUrl(`/catalogo/familia/${encodeURIComponent(landing.slug || slug)}`);
...
export default async function FamiliaSeoPage({ params, searchParams }) {
  const slug = params?.slug || "";
  ...
  return (
    <>
      ...
      <CatalogSeoLandingClient tipo="familia" data={data} slug={data.landing.slug || slug} />
    </>
  );
}
